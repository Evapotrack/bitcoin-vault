...
export function VaultBrowser() {
  ...
  const [accessBlocked, setAccessBlocked] = useState<{ name: string; costSats: number; frequency: string } | null>(null);
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
            ...
            <div key={file.id}
              ...
              onDoubleClick={async () => {
                ...
                if (result && result.protected) {
                  setAccessBlocked({ name: file.name, costSats: result.costSats!, frequency: result.frequency! });
                }
              }}>
      ...
      {/* Access blocked — file is protected */}
      {accessBlocked && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" role="dialog" aria-modal="true">
          <div className="bg-gray-900 rounded-xl p-6 max-w-sm w-full mx-4 space-y-4 text-center">
            ...
            <p className="text-gray-400 text-sm">
              &quot;{accessBlocked.name}&quot; requires a Bitcoin payment to access.
            </p>
            <div className="text-orange-400 font-mono text-2xl font-bold">
              {accessBlocked.costSats.toLocaleString()} sats
            </div>
            <p className="text-gray-500 text-xs">per {accessBlocked.frequency?.replace('-', ' ')}</p>
