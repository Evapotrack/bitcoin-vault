...
export function VaultBrowser() {
  ...
  const handleOpenFile = async (fileId: string) => {
    await window.bitcoinVault.openFile(fileId);
  };
    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          {currentFolderId && (
            <button onClick={navigateUp} className="text-gray-400 hover:text-white transition-colors text-sm">
              &larr; Back
            </button>
          )}
      ...
      {showNewFolder && (
        <div className="mb-4 flex gap-2">
          <input
            ...
            className="flex-1 bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-orange-500"
            autoFocus
          />
          ...
          {files.map(file => (
            ...
            >
              ...
              <div className="flex items-center gap-4">
                <span className="text-gray-500 text-xs whitespace-nowrap">{formatSize(file.originalSize)}</span>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-gray-900 rounded-xl p-6 max-w-sm w-full mx-4 space-y-4">
            ...
            <p className="text-gray-400 text-sm">
              &quot;{deleteTarget.name}&quot; will be permanently deleted from the vault.
